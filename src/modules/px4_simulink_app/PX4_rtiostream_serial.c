    ...
    while(count < argc) {
        ...
        if (option != NULL) {
            ...
            if ((strcmp(option, "-port") == 0) && (count != argc)) {
                ...
                strcpy(portName, portNameOrNum); //hard-coded for now to by-pass bug with sscanf with NuttX
...
static int serialPortOpenInitialize(SerialCommsData* sd, const int streamID,int argc, void* argv[])
{
    ...
    if (serialHandle == INVALID_HANDLE_VALUE) {
...
#else/*UNIX*/
        warnx("Opening serial port '%s' at baud %lu \n", sd->port,(unsigned long) sd->baud);
        int retries = 0;
        while (retries < 15) {
        ...
        warnx("Opened serial port '%s' RETURN:  %d \n", sd->port,serialHandle);
...
int rtIOStreamClose(int streamID)
{
    ...
    if (sd->verbosity) {
       ...
       warnx("rtIOStreamClose (connection id %d)\n", streamID);
